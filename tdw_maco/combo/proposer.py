...
class Proposer:
    ...
    def run(self, img_path, agent_id, save_path, progress, available_plans, steps, action_history, episode):
        ...
        # Get base prompt and replace placeholders
        ...
        for responses in responses_list:
            next_action = None
            try:
                # Extract next action from the response
                # Handle various formats: "Next action:", "My next action", "Next action (recommended):", etc.
                response_lower = responses.lower()
            ...
            all_proposes.append([next_action])
        return all_proposes
